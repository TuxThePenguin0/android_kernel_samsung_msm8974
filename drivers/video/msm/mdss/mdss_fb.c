...
static int mdss_fb_alloc_fbmem_iommu(struct msm_fb_data_type *mfd, int dom)
{
	...
	if (!virt) {
		...
		/* This always fails during early startup, but works later */
		/* mdss_fb_alloc_fbmem_iommu: unable to alloc fbmem size=1f00000 */
		/* probably because of iommu is not initialized early enough? */
		return -ENOMEM;
	}
